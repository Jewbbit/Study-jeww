(()=>{
  "use strict";
  const VERSION="2026-09-15-login-safari-v1";
  ...
  function toast(msg){
    ...
    clearTimeout(el.__sjLoginTimer);el.__sjLoginTimer=setTimeout(()=>el.classList.remove("show"),3600);
  }
  function shortCode(e){return String(e?.code||e?.name||"unknown").replace(/^auth\//,"")}
  ...
  async function login(){
    ...
    try{
      ...
      catch{try{await authMod.setPersistence(auth,authMod.browserSessionPersistence)}catch{}}
      const provider=new authMod.GoogleAuthProvider();
      provider.setCustomParameters({prompt:"select_account"});
      await authMod.signInWithPopup(auth,provider);
      toast("로그인되었습니다");
    }catch(e){
      console.error("Safari Google login failed",e);
      const code=shortCode(e);
      if(code==="popup-blocked")toast("로그인 실패 · Safari 팝업 차단");
